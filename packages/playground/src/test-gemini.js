...
async function runTest() {
    ...
    const registry = new llm_core_1.LLMProviderRegistry();
    registry.register(new llm_provider_gemini_1.GeminiProvider());
    ...
    const manager = new llm_core_1.LLMManager(mockStorage, registry);
    // 4. Get Provider
    ...
    try {
        const stream = provider.generateContentStream({ apiKey, modelId: 'gemini-1.5-flash' }, [{ role: 'user', parts: [{ text: 'Hello, this is a test from the HCS Monorepo. Please reply with "OK" if you receive this.' }] }], 'You are a testing script.', {});
